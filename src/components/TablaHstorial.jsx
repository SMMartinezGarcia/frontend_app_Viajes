...
export const TablaHistorial = ({ usuarioId }) => {
  ...
  const cargarHistorial = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/ubicaciones?usuarioId=${usuarioId}`);
  ...
  const eliminar = async (id) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/ubicaciones/${id}?usuarioId=${usuarioId}`);
      cargarHistorial();
