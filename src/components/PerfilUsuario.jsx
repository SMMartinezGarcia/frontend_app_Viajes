...
export const PerfilUsuario = ({ usuario }) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      axios.get(`${import.meta.env.VITE_API_URL}/api/viajes/reservas?usuarioId=${userId}`).catch(() => ({ data: [] })),
      axios.get(`${import.meta.env.VITE_API_URL}/api/ubicaciones?usuarioId=${userId}`).catch(() => ({ data: [] })),
